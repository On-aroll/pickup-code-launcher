# iPhone 快捷指令配置

苹果快捷指令的分享文件必须由 Apple 设备签名，因此需要在 iPhone 上完成最后创建。

## 菜鸟身份码

重复上面的步骤，URL 使用：

```text
cainiao://desktop/station_code
```

如果最新版 iOS 菜鸟不能识别该内部路径，改用网页：

```text
https://market.m.taobao.com/app/cn-yz/multi-activity/authCode.html?bizEntry=ALIPAY_GUOGUO
```

## 菜鸟包裹

重复上面的步骤，URL 使用：

```text
cainiao://
```

## 淘宝身份码

重复上面的步骤，URL 使用：

```text
taobao://m.taobao.com/tbopen/index.html?h5Url=https%3A%2F%2Fmarket.m.taobao.com%2Fapp%2Fcn-yz%2Fmulti-activity%2FauthCode.html
```

## 淘宝订单

重复上面的步骤，URL 使用：

```text
taobao://m.taobao.com/tbopen/index.html?h5Url=https%3A%2F%2Fh5.m.taobao.com%2Fmlapp%2Folist.html%3FtabCode%3DwaitConfirm
```

如果最新版 iOS 淘宝不能识别该内部路径，改用网页：

```text
https://h5.m.taobao.com/mlapp/olist.html?tabCode=waitConfirm
```

## 拼多多身份码

先测试：

```text
pinduoduo://com.xunmeng.pinduoduo/mdkd/package?tab=ID_CODE&entry_source=11
```

如果最新版 iOS 拼多多不能识别该内部路径，改用：

```text
https://m.pinduoduo.net/mdkd/package?entry_source=18&extra_params=from_wx_jump%3D1&p_channel=0
```

## 拼多多待取

重复上面的步骤，URL 使用：

```text
pinduoduo://com.xunmeng.pinduoduo/orders.html?type=3&comment_tab=1&combine_orders=1&main_orders=1&refer_page_name=personal
```

如果最新版 iOS 拼多多不能识别该内部路径，改用网页：

```text
https://mobile.yangkeduo.com/orders.html
```

## 抖音订单

重复上面的步骤，URL 使用：

```text
snssdk1128://
```

优先唤起已安装的抖音商城 App（包名 com.ss.android.ugc.livelite），未安装时回退抖音主 App。
抖音订单能力官方未开放直达参数，无法一步进入订单列表，打开后请点左上角「我的订单」。
iOS 快捷指令无法指定「待收货」分页，需在 App 内手动切换。

## 快手订单

重复上面的步骤，URL 使用：

```text
kwai://merchanthome
```

快手主 App 使用 kwai://，快手极速版使用 ksnebula://。

快手未公开订单页深链，该链接会打开快手小店买家端，进入后点「我的订单」即可。

## 哔哩哔哩订单

重复上面的步骤，URL 使用：

```text
https://mall.bilibili.com/neul/index.html?page=box_me&noTitleBar=1
```

该链接为 B 站会员购「我的」页，包含全部订单、待支付、待收货等分页，在 B 站 App 内打开时自动使用登录态。

## 分享给其他人

在快捷指令详情中点击“共享”，选择“复制 iCloud 链接”。获得五个链接后，可以把它们放进项目网页或视频简介。
